...
def tdrCanvas(canvName, x_min, x_max, y_min, y_max, nameXaxis, nameYaxis, square=kRectangular, iPos=11, is2D=False, isExtraSpace=False, lumitag='138 fb^{-1} (13 TeV)', margins=(None, None, None, None), maxdigits=(None, None)):
  ...
  if margins[0] is not None: canv.SetTopMargin(margins[0])
  if margins[1] is not None: canv.SetRightMargin(margins[1])
  if margins[2] is not None: canv.SetBottomMargin(margins[2])
  if margins[3] is not None: canv.SetLeftMargin(margins[3])
...
def tdrDiCanvas(canvName, x_min, x_max, y_min, y_max, y_min2, y_max2, nameXaxis, nameYaxis, nameYaxis2, square=kRectangular, iPos=11, lumitag='138 fb^{-1} (13 TeV)'):
  setTDRStyle()
  ...
  CMS_lumi(rt.gPad, iPos, lumitag=lumitag)
...
def tdrSetHeader(leg, legTitle, textAlign=12, textSize=0.04, textFont=42, textColor=rt.kBlack):
  header = rt.TLegendEntry( 0, legTitle, "h" )
  header.SetTextFont(textFont)
  header.SetTextSize(textSize)
  header.SetTextAlign(textAlign)
...
def tdrDraw(h, opt, marker=rt.kFullCircle, mcolor=rt.kBlack, lstyle=rt.kSolid, lcolor=-1, fstyle=1001, fcolor=rt.kYellow+1, alpha=-1, lwidth=3, msize=1.0):
  HistCosmetics(h)
